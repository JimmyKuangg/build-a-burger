...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  function mousePressed(event) {
    ...
    if (
      game.whichSectionBoard(clickedAt.x, clickedAt.y) === 'board section 1' &&
      game.burger1.complete
    ) {
      game.draggingHamburger = true;
    } else if (
      game.whichSectionBoard(clickedAt.x, clickedAt.y) === 'board section 2' &&
      game.burger2.complete
    ) {
      game.draggingHamburger = true;
    } else if (
      game.whichSectionBoard(clickedAt.x, clickedAt.y) === 'board section 3' &&
      game.burger3.complete
    ) {
      game.draggingHamburger = true;
    }
    ...
    if (game.whichSectionGriddle(game.mouse.x, game.mouse.y) === 'section 1') {
      ...
      game.whichSectionGriddle(game.mouse.x, game.mouse.y) === 'section 2'
    ) {
      ...
      game.whichSectionGriddle(game.mouse.x, game.mouse.y) === 'section 3'
    ) {
  ...
  function mouseReleased(event) {
    ...
    if (game.draggingRaw) {
      if (game.whichSectionGriddle(game.mouse.x, game.mouse.y) === '') {
        ...
        game.patty1.assignSection(
          game.whichSectionGriddle(game.mouse.x, game.mouse.y)
        );
        game.patty1.cook();
      } else if (!game.patty2.cooking && !game.patty2.flipping) {
        game.patty2.assignSection(
          game.whichSectionGriddle(game.mouse.x, game.mouse.y)
        );
        game.patty2.cook();
      } else if (!game.patty3.cooking && !game.patty3.flipping) {
        game.patty3.assignSection(
          game.whichSectionGriddle(game.mouse.x, game.mouse.y)
        );
        game.patty3.cook();
      }
    ...
    if (game.draggingCooked) {
      ...
      if (
        game.whichSectionBoard(game.mouse.x, game.mouse.y) === 'board section 1'
      ) {
        ...
        game.whichSectionBoard(game.mouse.x, game.mouse.y) === 'board section 2'
      ) {
        if (clickedAt.x >= 725 && clickedAt.x <= 825) {
          //Check which patty is the one that is placed on griddle section 1
          if (game.patty1.griddleSection === 'section 1') {
        ...
        game.whichSectionBoard(game.mouse.x, game.mouse.y) === 'board section 3'
      ) {
        if (clickedAt.x >= 725 && clickedAt.x <= 825) {
          //Check which patty is the one that is placed on griddle section 1
          if (game.patty1.griddleSection === 'section 1') {
    ...
    if (game.draggingBun) {
      if (
        game.whichSectionBoard(game.mouse.x, game.mouse.y) ===
          'board section 1' &&
        game.burger1.hasPatty
      ) {
        game.burger1.addBun();
      } else if (
        game.whichSectionBoard(game.mouse.x, game.mouse.y) ===
          'board section 2' &&
        game.burger2.hasPatty
      ) {
        game.burger2.addBun();
      } else if (
        game.whichSectionBoard(game.mouse.x, game.mouse.y) ===
          'board section 3' &&
        game.burger3.hasPatty
      ) {
        game.burger3.addBun();
      }
    ...
    if (game.draggingHamburger) {
      if (
        game.inWindow(game.mouse.x, game.mouse.y) &&
        game.score(game.whichSectionBoard(clickedAt.x, clickedAt.y))
      ) {
      }
